...
class Environment(object):
    def __init__(
        ...
        base_dir=sys.path[0] + "/dataset_generation/dataset/",
    ):  # env_type if type(env_type) == list else [env_type]
    ...
    def compute_line_constraints(self, net):
        ...
        lines = lines_condition[
            lines_condition["loading_percent"] > lines_condition["max_loading_percent"]
        ]
    ...
    def save_initial_values(self, use_min_gen=True):
        ...
        if use_min_gen == True:
            self.generators_value = {
                key: value["min"] for key, value in self.gen_config.items()
            }
        else:
    ...
    def create_net_info(self):
        ...
        nodes_info = tf.cast(np.asarray(node_features_pre), dtype=tf.dtypes.float32)
        # trasnpone edges
    ...
    def get_state(self):
        ...
        node_features = (
            tf.reshape(
                node_features_pre,
                [node_features_pre.shape[0] * node_features_pre.shape[1]],
            )
            .numpy()
            .tolist()
        )
    ...
    def update_generators(self, gen_id):
        try:
            ...
            if self.generators_value_norm[bus_id] < self.generator_slices:
                ...
                self.generators_value[bus_id] = (
                    self.generators_value_norm[bus_id]
                    * (self.gen_config[bus_id]["max"] - self.gen_config[bus_id]["min"])
                    / self.generator_slices
                    + self.gen_config[bus_id]["min"]
                )
                return True
            else:
                return False
    ...
    def aument_one_all_generatos(self):
        self.generators_value_norm = {
            key: value + 1
            for key, value in self.generators_value_norm.items()
            if value < self.generator_slices
        }
    ...
    def save_values_logs_powers(self):
        ...
        self.generators_loads.append(
            [
                round(self.generators_value[x] / self.gen_config[x]["max"], 2) * 100
                for x in self.generators_value.keys()
            ]
        )
    ...
    ****************************************************************************
    ...
    def _compute_solution_cost(self, net):
        ...
        cost = (
            cost
            + cost_row["cp0_eur"].values[0]
            + net.res_ext_grid["p_mw"].values[0] * cost_row["cp1_eur_per_mw"].values[0]
            + net.res_ext_grid["p_mw"].values[0] ** 2
        ...
        for index, row in net.res_gen.iterrows():
            cost_row = net.poly_cost[
                (net.poly_cost["et"] == "gen") & (net.poly_cost["element"] == index)
            ]
            cost = (
                cost
                + cost_row["cp0_eur"].values[0]
                + cost_row["cp1_eur_per_mw"].values[0] * row["p_mw"]
