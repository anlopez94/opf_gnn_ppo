...
class PPOAgent(object):
    ...
    def __init__(
        ...
        optimizer=tf.keras.optimizers.Adam(
            learning_rate=0.0003, beta_1=0.9, epsilon=0.00001
        ),
        base_dir="logs",
    ...
    def define_horizon(self):
        self.horizon = int(
            len(self.env.generators_value)
            * self.env.generator_slices
            * self.env.horizon_percentage
        )
    ...
    def gae_estimation(self, rewards, values, last_value):
        ...
        for i in reversed(range(self.horizon)):
            ...
            advantages[i] = last_gae_lambda = (
                delta + self.gamma * self.gae_lambda * last_gae_lambda
            )
    ...
    def run_episode(self):
        ...
        states = np.zeros(
            (self.horizon, self.env.n_nodes * self.env.num_features), dtype=np.float32
        )
    ...
    def run_update(
        self, training_episode, states, actions, returns, advantages, log_probs
    ):
        ...
        for epoch in range(self.epochs):
            ...
            for start in range(0, len(actions), self.batch_size):
                ...
                minibatch_actions = tf.ragged.constant(
                    [actions[i] for i in list(minibatch_ind)]
                )
                minibatch_log_probs = tf.ragged.constant(
                    [log_probs[i] for i in list(minibatch_ind)]
                )
                actor_loss, critic_loss, loss, grads = self.compute_losses_and_grads(
                    states[minibatch_ind],
                    minibatch_actions,
    ...
    def train_and_evaluate(self):
        ...
        while not (
            self.env.num_sample == self.last_training_sample
            and self.change_sample == True
        ):
            training_episode += 1
            print(f"Episode {training_episode} starting {self.env.num_sample}")
            states, actions, rewards, log_probs, values, last_value, t = (
                self.run_episode()
            )
            ...
            actor_losses, critic_losses, losses = self.run_update(
                training_episode, states, actions, returns, advantages, log_probs
            )

            tf_logs.training_episode_logs(
                self.writer,
                self.env,
                training_episode,
                states,
            ...
            print(
                f"Episode {training_episode} finished con sample {self.env.num_sample}"
            )
            ...
            if (training_episode + 1) % self.eval_period == 0:
                ...
                if self.save_checkpoints:
                    ...
                    self.save_model(
                        os.path.join(
                            self.checkpoint_dir, "episode" + str(self.eval_episode)
                        )
                    )
                if (
                    self.change_load
                    and self.eval_episode % self.change_load_period == 0
                ):
    ...
    def only_evaluate(self):
        ...
        results_eval = Results(
            self.set_experiment_identifier(only_eval=True), "results"
        )
    ...
    def generate_eval_actor_critic_functions(self):
        ...
        for eval_env_type in self.eval_env_type:
            self.eval_actor[eval_env_type] = Actor(
                self.eval_envs[eval_env_type].graph_info
            )
            self.eval_actor[eval_env_type].build()
            self.eval_critic[eval_env_type] = Critic(
                self.eval_envs[eval_env_type].graph_info
            )
    ...
    def update_eval_actor_critic_functions(self):
        for eval_env_type in self.eval_env_type:
            for w_model, w_eval_actor in zip(
            ...
            for w_model, w_eval_critic in zip(
    ...
    def training_eval(self):
        ...
        for eval_env_type in self.eval_env_type:
            ...
            for _ in range(self.num_eval_samples):
                ...
                tf_logs.eval_step_logs(
                    self.writer, self.eval_envs[eval_env_type], self.eval_step, state
                )
                probs, values = [], []

                for i in range(self.horizon):
                    self.eval_step += 1
                    actions, log_probs = self.eval_act(
                        self.eval_actor[eval_env_type],
                        state,
            ...
            tf_logs.eval_top_log(
                self.writer, self.eval_episode, min_costs, eval_env_type
            )
        self.eval_episode += 1
    ...
    def compute_actor_loss(self, new_log_probs, old_log_probs, advantages):
        ...
        pg_loss_2 = tf.map_fn(
            lambda x: -x[0]
            * tf.clip_by_value(x[1], 1.0 - self.clip_param, 1.0 + self.clip_param),
            (advantages, ratio),
            fn_output_signature=tf.RaggedTensorSpec(shape=[None], dtype=tf.float32),
        )  # - advantages * tf.clip_by_value(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param)
    ...
    def get_new_log_prob_and_entropy(self, state, actions):
        ...
        log_probs = tf.map_fn(
            lambda x: probs.log_prob(x), actions, fn_output_signature=tf.float32
        )  # [probs.log_prob(action) for action in actions]
        return (log_probs, probs.entropy())

    # @tf.function
    def compute_losses_and_grads(
        self, states, actions, returns, advantages, old_log_probs
    ):
        with tf.GradientTape(persistent=True) as tape:
            new_log_probs, entropy = tf.map_fn(
                lambda x: self.get_new_log_prob_and_entropy(x[0], x[1]),
                (states, actions),
                fn_output_signature=(
            ...
            values = tf.map_fn(
                lambda x: self.training_critic[self.env.network](x, training=True),
                states,
            ...
            loss = (
                actor_loss
                - self.entropy_loss_factor * entropy_loss
                + self.critic_loss_factor * critic_loss
            )
        ...
        if select_max:
            ...
            actions = list(
                set(
                    [
                        probs.sample().numpy()
                        for _ in range(self.max_simultaneous_actions)
                    ]
                )
            )
    ...
    def load_latest_model(self, previous_env_type):
        for w_model, w_actor in zip(
            self.training_actor[previous_env_type].trainable_variables,
            self.training_actor[self.env.network].trainable_variables,
        ):
            w_actor.assign(w_model)
        for w_model, w_critic in zip(
            self.training_critic[previous_env_type].trainable_variables,
            self.training_critic[self.env.network].trainable_variables,
        ):
    ...
    def set_experiment_identifier(self, only_eval=False):
        ...
        if mode == "training":
            ...
            # pre = 'MESS'
            # num_actions = 'actions' + str(self.max_simultaneous_actions)
            create_message = "create_message" + str(
                self.training_actor[self.env.network].create_message_nn
            )
            generator_slices = "generator_slices" + str(self.env.generator_slices)
            ...
            action_folder = ("-").join(
                [create_message, generator_slices, horizons, reward, reward_computation]
            )
            ...
            node_state_size = "node_state_size" + str(
                self.training_actor[self.env.network].node_state_size
            )
            message_iterations = "message_iterations" + str(
                self.training_actor[self.env.network].message_iterations
            )
            agent_folder = ("-").join(
                [
                    batch,
            ...
            self.experiment_identifier = os.path.join(
                mode, env_folder, action_folder, agent_folder, function_folder
            )
